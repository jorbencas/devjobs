...
KEY=${1:-prueba_upload}
SRC=${2:-/home/jorge/dev/devjobs/test_videos/test/.placeholder}
DEST=/home/jorge/dev/devjobs/Videos/comprimidos/sendo_KW_${KEY}_compressed.mp4

# Buscar un mp4 de ejemplo si no se pasó uno explícito
if [ ! -f "$SRC" ]; then
    SRC=$(find /home/jorge/dev/devjobs/test_videos -maxdepth 2 -name "*.mp4" 2>/dev/null | head -1)
fi
if [ -z "$SRC" ]; then
